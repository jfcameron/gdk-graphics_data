...
static inline decltype(STBI_rgb_alpha) textureFormatToSTBImageFormat(const texture_data::format a) {
    switch(a) {
        case texture_data::format::rgba: return STBI_rgb_alpha;
        case texture_data::format::rgb:  return STBI_rgb;
        case texture_data::format::grey: return STBI_grey;
        default: break;
    }
    throw exception("unhandled texture format to STBI format case");
}

static inline char textureFormatToChannelCount(const texture_data::format a) {
    switch(a) {
        case texture_data::format::rgba: return 4;
        case texture_data::format::rgb:  return 3;
        case texture_data::format::grey: return 1;
        default: break;
    }
...
static inline std::pair<view, std::shared_ptr<channel_data>> decodePNG(
    const encoded_byte *aDataStart, const size_t aLength, const texture_data::format aFormat = texture_data::format::rgba) {
    ...
    if (std::unique_ptr<channel_type, std::function<void(channel_type *)>> decodedData(
        ...
        data.width = width;
        data.height = height;
        data.format = texture_data::format::rgba;
...
std::pair<view, std::shared_ptr<channel_data>> gdk::graphics::ext::make_from_png(
    std::span<const encoded_byte> aPNGData, const texture_data::format aFormat) {
