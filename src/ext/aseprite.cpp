...
namespace gdk::graphics::ext {
    ...
    aseprite_sheet read_aseprite(const std::span<const std::byte> aBytes) {
        ...
        if (const auto *const pList = std::get_if<std::shared_ptr<array>>(&pFrames->held)) {
            if ((*pList)->size() > MOST_ASEPRITE_FRAMES) throw wrong("a sheet of more frames than are allowed");
            ...
            if ((*pKeyed)->size() > MOST_ASEPRITE_FRAMES) throw wrong("a sheet of more frames than are allowed");
        ...
        if (const auto *const pTags = within(*pMeta, "frameTags")) {
            ...
            if ((*pList)->size() > MOST_ASEPRITE_FRAMES) throw wrong("a sheet of more tags than are allowed");
